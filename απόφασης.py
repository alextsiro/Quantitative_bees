...
def clicked_ok():
    ...
    for i in range (0,21):
        model.Con.add(expr = model.w[i,0]==arxika_melissia[i])    
    ...
    for p in range (0,vimata):
        for i in range (3,21): 
            model.Con.add(expr = model.prin[i+anaptyxi[i],p+1]==model.w[i,p])
        for i in range (3,21+max_anaptyxi):   
            if test[i]==0:
    ...
    for p in range (1,vimata+1):
        for i in range (3,21): 
            model.Con.add(expr = model.w[i,p]>=0)
            model.Con.add(expr = model.w[i,p]>=model.meta[i,p]+s*model.apofasis[i,p]-s)
            model.Con.add(expr = model.w[i,p]<=model.apofasis[i,p]*s)
            model.Con.add(expr = model.w[i,p]<=model.meta[i,p])  

    #περιορισμός μελισσοσηνών 1 δυναμικότητας
    for p in range (1,vimata+1):
        model.Con.add(expr = sum(model.apofasis[i,p] for i in range (3,21))==1)
        
    #περιορισμοί αναδιάταξης : πλαισίων και βασσιλισσών            
    for p in range (1,vimata+1):   
        #πλαίσια         
        model.Con.add(expr = sum(i*model.prin[i,p] for i in range (0,21+max_anaptyxi))==model.nea[p]+sum(i*model.w[i,p] for i in range (0,21))) 
    #βασσίλισσες
        model.Con.add(expr = s==sum(model.w[i,p] for i in range (0,21)))

    for p in range (1,vimata+1):
        model.Con.add(expr = sum(model.apofasis[i,p] for i in range(0,21))==1)
       
#περιορισμός αποδοτικών μελισσοσμηνών κατα το πέρας της μελισσοκομικής άνοιξης        
    model.Con.add(expr = s ==model.w[20,vimata])
    ...
    for p in range (1,vimata+1):
        ...
        for i in range (3,21):           
            lbl = Label(window, text=int(model.w[i,p].value))
